...
use app\modules\siteadmin\models\ComunicadoSearch;
...
class ComunicadoController extends Controller
{
    ...
    $model->edital_id = $session['sess_abertura'];
        ...
        if ($model->load(Yii::$app->request->post()) && $model->save()) {
            ...
            $model->file = UploadedFile::getInstance($model, 'file');
                ...
                if(empty($model->file)){
                Yii::$app->session->setFlash('danger', 'É obrigatório o envio do arquivo para inserir o Comunicado! ');
 ...
 if($codunidade == 11){
        ...
        if ($model->load(Yii::$app->request->post()) && $model->save()) {
            ...
            Yii::$app->session->setFlash('success', 'Comunicado atualizado com sucesso! ');
    ...
    {
        if (($model = Comunicado::findOne(['id' => $id])) !== null) {
            return $model;
        } else {
